...
- **`name`**: a required, non-null top-level display name for the record (see per-module rules below).
...
**Important:** On this branch every `run_tool` invocation that carries a `server_name` and `tool_name` produces an audit event; there is no read-only/mutating classification. The **reporting layer (AAP) must filter by `server_name`** to distinguish infrastructure nodes (e.g. AWS, Azure, GCP servers managing VMs, storage, networking) from non-infrastructure entity changes (e.g. GitHub servers managing issues, PRs, repositories). Not every MCP server manages infrastructure -- see the `server_name` field in audit events for downstream filtering.
