...
# Known, tracked gap -- deliberately not fixed in this change.
#
# ansible.mcp emits `device_type` and `infra_bucket` but no `infra_type`, because
# an MCP server is neither `public_cloud` nor `private_cloud` and the taxonomy has
# no value that fits. That is a decision for the taxonomy owner, not for this PR;
# it is tracked as AAP-92622.
#
# `strict=True` is deliberate: when `infra_type` is added, this test starts
# passing and pytest fails on the unexpected pass, which tells whoever fixed it
# to delete this marker rather than leaving a permanently-xfailing test behind.
TAXONOMY_GAP = pytest.mark.xfail(
    strict=True,
    reason="AAP-92622: infra_type is not yet defined for MCP servers",
)
